...
bool  Deploy::validate() {
    if (*owner->armies < troops) {
        if (dynamic_cast<HumanPlayerStrategy*>(owner->ps) != nullptr)
...
Advance::Advance(Player* p, Territory* Source, int t, Territory* Destination, Command* c, LogObserver* l,bool cheat) : Order(p, c, l)
{
    set_type_id(1);
    cheater = cheat;
    troops = t;
...
ostream& operator<<(ostream& strm, const Advance& a) {
 ...
 bool Advance::execute() {
     ...
     Player* ifneutral = owner->getplayer(owner->AllPlayers, *Desti->player_owner);

     if (dynamic_cast<NeutralPlayerStrategy*>(ifneutral->ps) != nullptr) {
         ifneutral->ps = new AggressivePlayerStrategy(ifneutral);
         cout << "since this is an attack on the neutral player he got angry and will become aggresive from now on!!!!!! " << endl;
     }
