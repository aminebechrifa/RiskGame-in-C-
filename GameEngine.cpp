...
// constructor
GameEngine::GameEngine(bool fromfile,string filename)
{
    lo = new LogObserver();
    Attach(lo);
    if (fromfile)
    Cp = new CommandProcessorAdapter(new FileLineReader(),/* filename*/"C:\\Users\\bechr\\Documents\\GitHub\\COMP345-WarzoneProject\\all\\working\\1.txt", lo);
    else 
    ...
    issueorders->addpossible_action("issueorder", "endissueorders", "start");
...
GameEngine::GameEngine()
{
    ...
    NeutralPlayer = new Player("NEUTRAL");
// insert the possible commands in a list corresponding to each of the states
    start->addpossible_action("loadmap", "tournament", "");
...
string GameEngine::add_player(string s,string strat) {
    if (getPlayer(all_players, s) != nullptr) {
        return "Player already exist";
    }
    Player* p = new Player(s,strat);
    p->cP = Cp;
    all_players.insert(all_players.begin(), p);
    return "";
}
...
string  GameEngine::transition(string com, string arg)
{
    if (com == "addplayer") {
        ...
        string strat = string(arg.substr(0, arg.find(" ")));

       string  name = arg.substr(arg.find(" ") + 1);
        if (name== strat) {
            problem = add_player(name,"human");
        }
        else {
            if (strat == "human") {
                problem = add_player(name,strat);
            }
                ...
                currentCommand->setEffect("Uknown strat entered ");
                return "Uknown strat entered";
            }
...
string GameEngine::issueOrderPhase() {
    while (true) {
        ...
        cout << endl <<"the first issueorder of the turn ended please type continue to make another issueorders phase" << endl;
        currentCommand = Cp->getCommand();
        if (currentCommand->getCommand() != "continue") {
            return "";
        }
    }
    return "";
...
void GameEngine::wipe() {
    State* start =getstate("start");
    current_state = start;
     map=nullptr;
     deck = new Deck(5000);
...
void GameEngine::mainGameLoop() {
    ...
    while (true) {
        ...
        string message = "";
        cout << "current state is : " + *current_state->name << endl;
        if (*current_state->name != "executeorders") {
            currentCommand = Cp->getCommand();
            ...
            if (*current_state->name == "issueorders") {
                if (com == "replay") {
                    cout << endl << endl << endl << "current game ended we will start another game " << endl << endl << endl << endl;
                    wipe();
                    StartupPhase();
                    mainGameLoop();
